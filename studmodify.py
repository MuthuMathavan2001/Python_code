...
def delete():
    ...
    cursor=db.cursor()
    regno=reg.get()
    db.query("delete from stud where Roll=" + regno)
    messagebox.showinfo("Print","Deleted sucessfully")
    db.commit()
    db.close()
def modify():
    try:
    ...
    db.query("update stud set Roll='"+ regno +"',Tamil='"+ f +"',English='"+ g +"',Maths='"+ h +"',Science='"+ i +"',SocialScience='"+ j +"',Total='"+ k +"',percentage='"+ l +"',result='"+ result +"',Grade='"+ Grade +"' where Roll=" +regno)
    db.commit()
    messagebox.showinfo("Student","Successful")
    db.close()
...
btn1=Button(frm, text="Delete", fg="red", font=("Tahoma",10),command=delete)
btn1.place(x=100,y=350)

btn2=Button(frm, text="Modify", fg="red", font=("Tahoma",10),command=modify)
